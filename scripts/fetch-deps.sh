#!/bin/bash
# Скачивает исходники crispy-doom (движок; форк chocolate-doom) и SDL2-библиотек,
# создаёт симлинки для ndk-build.
set -e
cd "$(dirname "$0")/.."
...
if [ ! -d crispy-doom ]; then
  git clone --depth 1 https://github.com/fabiangreffrath/crispy-doom.git
fi
